...
beneficios_df = beneficios_df.loc[beneficios_df['EstadoBeneficio_id'] != 'Desestimado']
...
beneficios_df['MotivoBeneficio_id'] = np.where(beneficios_df['MotivoBeneficio_id']==1, 'Programas de Capacitación', 
                                       ...
                                       np.where(beneficios_df['MotivoBeneficio_id']==4, 'Materiales | Publicaciones | Insumos | Difusión', 
...
beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.strip()
beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.replace("\n", "")
beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.replace("\r", "\t")

beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.upper()
beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.replace('"', '')
beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.replace(',', '')
beneficios_df['Observaciones'] = beneficios_df['Observaciones'].str.strip()
...
beneficios_df = beneficios_df.drop(['ResumenEjecutivo', 'FechaPedido','FechaAlta','Referente','EmailRef','FechaEstado', 
                                    'Vitalicio','DetalleEstado','RindeComprobante','Observaciones','Descripcion','Nombre_Razon',
                                    'FechaBaja','DetalleBaja','Alias','Multiples', 'Usuario_id','Consejero_id','TipoReferente_id', 
                                    'ItemPresupuesto_id','Activo','SolicitudAdministrativa','Apellido','FechaNacimiento',
                                    'NumDocumento','Cuil_Cuit','TipoDocumento_id','Relacion','Domicilio_id','Personeria',
                                    'InstitucionEducativa','Cobertura','Otros','Exenta','Fecha_Alta','EstadoSaludFamilia_Id',
                                    'Trabaja','PadreMadreBecado','Activo','Genero','EstudiosAlcanzados_Id','AnoAlcanzado',
                                    'Tel','Cel','Email','SearchName'], axis=1)

print(beneficios_df)
beneficios_df.to_csv('beneficios_programas.csv', index = False)

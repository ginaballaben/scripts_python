...
benef_1 = """  SELECT distinct  Becado.Id AS id_db__c, Becado.Nombre_Razon AS FirstName, Becado.Apellido AS LastName, Becado.FechaNacimiento AS Birthdate, Becado.NumDocumento AS DNI__c, Becado.Cuil_Cuit AS Cuil__c,
...
benef_df1 = datos(benef_1, 'Beneficios')
# print(benef_df1)
# benef_df1.to_csv("benef_1.csv", index = False)

benef_df2 = datos(benef_2, 'Beneficios')
# benef_df2.to_csv("benef_2.csv", index = False)
# print(benef_df2)

benef_todos = """ SELECT distinct  Becado.Id AS id_db__c, Becado.Nombre_Razon AS FirstName, Becado.Apellido AS LastName, Becado.FechaNacimiento AS Birthdate, Becado.NumDocumento AS DNI__c, Becado.Cuil_Cuit AS Cuil__c,
				Becado.Genero AS Genero__c, Becado.Tel AS HomePhone, Becado.Cel AS MobilePhone, Becado.Email AS Email, Domicilio.Direccion AS MailingStreet,
				Localidad.Nombre AS MailingCity, Provincia.Nombre AS MailingState, CodigoPostal.Nombre AS MailingPostalCode, EstudiosAlcanzados.Descripcion AS Maximo_nivel_educativo_alcanzado__c, FormaPagoBeneficiario.Banco, FormaPagoBeneficiario.CBU AS CBU__c,
				BENEF.Id AS id_parents, InstitucionEducativa.Id AS id_escuelas
...
benef_df = datos(benef_todos, 'Beneficios')
#benef_df.to_csv("benef_todos.csv", index = False)
...
benef_sf = pd.read_csv(r'C:\Users\NAV2019VOXA\Downloads\ZIGLA\FPC\beneficiaries_sf.csv', encoding = 'utf-8-sig', sep = ';')

print(benef_sf)

benef_sf_dni = benef_sf_dni[['id_db__c']]

benef_sf_dni.rename(columns={'id_db__c': 'id_mig'}, inplace=True)

benef_df = benef_df.merge(benef_sf_dni, left_on='id_db__c', right_on='id_mig', how='left')
...
benef_df = benef_df.drop_duplicates(benef_df.columns[benef_df.columns.isin(['id_db__c'])],
                              keep='last')
print(benef_df)
benef_df.to_csv('benefic_match1_dni.csv', index = False)
...
print(benef_df)
benef_df.to_csv('benef_match_2.csv', index = False)
